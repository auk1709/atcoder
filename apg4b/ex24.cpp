...
struct Clock {
  ...
  string to_str() {
    stringstream t;
    t << setw(2) << setfill('0') << hour << ':' << setw(2) << setfill('0')
  ...
  void shift(int diff_second) {
    int before = hour * 3600 + minute * 60 + second;
    int after = before + diff_second;
    while (after < 0) after = 86400 + after;
    hour = after / 3600 % 24;
    minute = (after % 3600) / 60;
    second = (after % 3600) % 60;
  }
};
// -------------------
// ここから先は変更しない
